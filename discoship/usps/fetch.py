...
from discoship.usps.cpg import fetch_cpg_data, insert_cpg_data
from discoship.usps.fcpis import fetch_fcpis_rates_data, insert_fcpis_rates_data
from discoship.usps.pmi import fetch_pmi_rates_data, insert_pmi_rates_data
#from discoship.usps.pmei import fetch_pmei_rates_data, insert_pmei_rates_data
    ...
    * `service` is one of: FCPIS, PMI, possibly PMEI.  If `fetchall` is not
    ...
    if fetchall or rates:
        ...
        if fetchall or service == USPS_SVC_FCPIS:
            ...
            insert_fcpis_rates_data(fcpis_rates_data)
        # Priority Mail Int'l
        if fetchall or service == USPS_SVC_PMI:
            pmi_rates_data = fetch_pmi_rates_data()
            insert_pmi_rates_data(pmi_rates_data)
        # Priority Mail Express Int'l
        if fetchall or service == USPS_SVC_PMEI:
            #pmei_rates_data = fetch_pmei_rates_data()
            #insert_pmei_rates_data(pmei_rates_data)
            pass
